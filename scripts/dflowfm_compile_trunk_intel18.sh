...
if [ ! -f `which ifort` ]; then
   echo 'Please set ifort first!'
   exit 1
else
   export IFORTEXE=`which ifort`
   export IFORTDIR=`dirname $IFORTEXE`
   #export IFORTLIB=${IFORTDIR}/../../compiler/lib/intel64
   if [ ! -d $IFORTLIB ]; then
      echo "Could not find ifort libraries at $IFORTLIB"
      exit 1
   fi
fi
...
export DFLOWFMROOT=${BASE}/dflowfm_linux64_ifort
...
#netcdf
export NETCDFROOT=${BASE}/netcdf_linux64_ifort
...
#petsc
export PETSC_ROOT=$BASE/petsc_linux64_ifort

#metis
export METIS_ROOT=$BASE/metis_linux64_ifort
...
FFLAGS="$OPTIM -I$NETCDFROOT/include/ -I$MPI_ROOT/include/ -I$METIS_ROOT/include" \
     ...
     ./configure --prefix=$DFLOWFMROOT --with-mpi --with-petsc --with-metis=${METIS_ROOT} 2>&1 |tee myconfig.log
...
echo 'cp $DFLOWFMROOT/share/dflowfm/unstruc.ini .' >>$DFLOWFMROOT/bin/dflowfm.sh
echo '' >>$DFLOWFMROOT/bin/dflowfm.sh
...
echo '' >>$DFLOWFMROOT/bin/partition.sh
echo 'cp $DFLOWFMROOT/share/dflowfm/unstruc.ini .' >>$DFLOWFMROOT/bin/partition.sh
echo '' >>$DFLOWFMROOT/bin/partition.sh
